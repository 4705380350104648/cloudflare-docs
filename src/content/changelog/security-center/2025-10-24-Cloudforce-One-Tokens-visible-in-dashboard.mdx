---
title: Cloudforce One RFI tokens are now visible in the dashboard
description: The Requests for Information (RFI) dashboard now shows the number of tokens used for each RFI
products:
  - Cloudforce One
date: 2025-10-24
---

The Requests for Information Dashboard now shows users the number of tokens used by each submitted RFI, in order to better understand usage of tokens and how they relate to each request submitted. 

![Cloudforce One RFI tokens](~/assets/images/changelog/security-center/2025-10-24RFITokens.png)

What's new:

- Users can now see the number of tokens used for a submitted request for information.
- Users can see the remaining tokens allocated to their account for the quarter.
- Users can only select the Routine priority for the `Strategic Threat Research` request type.

Cloudforce One subscribers can try it now in [Application Security > Threat Intelligence > Requests for Information](https://dash.cloudflare.com/?to=/:account/security-center/threat-intelligence/requests).
